...
function buildScript(origin: string): string {
  ...
  const lines: string[] = [
    ...
    `fi`,
    ``,
    `# Resolve \`claude\` at install time. The LaunchAgent runs with a`,
    `# sparse PATH that excludes ~/.local/bin (where Claude Code typically`,
    `# installs), so we bake the absolute path into the env + plist.`,
    `if ! CLAUDE_BIN="${S}(command -v claude 2>/dev/null)"; then`,
    `  if [[ -x "${S}HOME/.local/bin/claude" ]]; then`,
    `    CLAUDE_BIN="${S}HOME/.local/bin/claude"`,
    `  else`,
    `    echo "error: claude CLI not found on PATH or at ~/.local/bin/claude." >&2`,
    ...
    `SUPABASE_SERVICE_ROLE_KEY="${S}(${nodeExtract("supabase_service_role_key")})"`,
    ...
    `    <key>ROVE_SUPABASE_SERVICE_ROLE_KEY</key>    <string>${S}SUPABASE_SERVICE_ROLE_KEY</string>`,
    ...
    `    <key>ROVE_CLAUDE_BIN</key>                   <string>${S}CLAUDE_BIN</string>`,
    `  </dict>`,
